...
class MyTreeGroupPatch extends CGFobject {
	...
	initBuffers(scene) {
		...
		this.tree1 = new MyTree(
			scene,
			Math.random() * 3 + 0.5,
			trunkRadius,
			Math.random() * 4 + 0.8,
			Math.random() * 1 + trunkRadius + 0.1,
			this.textureTrunk,
			this.textureTree
		);
		trunkRadius = Math.random() * 0.3 + 0.2;
		this.tree2 = new MyTree(
			scene,
			Math.random() * 3 + 0.5,
			trunkRadius,
			Math.random() * 4 + 0.8,
